...
export default async function Home({ searchParams }: { searchParams: Promise<{ message?: string }> }) {
  ...
  return <main className="site-shell"><Navigation/><header className="hero"><span className="eyebrow">Southern California · Round1</span><h1>maimai Queue Tracker</h1><p className="rounded-content">Check current maimai DX queue times. Updates are crowdsourced and can change quickly.</p></header>{query.message && <div className="notice success" role="status">{query.message}</div>}<div className="section-head"><h2>Round1 locations</h2><span>{locations.length ? `${locations.length} locations` : "Community reported"}</span></div>{loadFailed ? <div className="empty-state"><h3>Can’t load locations right now</h3>Please check your connection or try again soon.</div> : locations.length === 0 ? <div className="empty-state"><h3>No locations available yet</h3>Locations will appear here when configured in the database.</div> : <div className="location-list">{locations.map(loc => { const previousUpdate = loc.latest_update; const update = previousUpdate && isCurrentStatus(previousUpdate.created_at) ? previousUpdate : null; const stale = update ? age(update.created_at) : null; return <article className="location-card" key={loc.id}><div className="card-foot"><div><div className="loc-name rounded-content">Round1 {loc.name}</div><div className="loc-city rounded-content"><MapPin size={12} style={{display:"inline",verticalAlign:"-2px"}}/> {loc.city}, California</div></div>{approved && <Link className="update-link" href={`/update?location=${loc.id}`}><Gamepad2/> Update</Link>}</div>{update ? <><div className="counts rounded-content"><div className="count-pill play"><Gamepad2/>{update.playing_count} playing</div><div className="count-pill queue"><Users/>{update.queue_count} waiting</div></div><div className="updated rounded-content"><span><i className={`stale-dot ${stale?.cls ?? ""}`}/><Clock3 size={13} style={{display:"inline",verticalAlign:"-2px"}}/> Updated {stale?.text} by <strong>{update.display_name}</strong></span></div></> : <><div className="empty-status rounded-content">{statusReadFailed ? "Status temporarily unavailable." : "No recent status reported yet."}</div><div className="updated rounded-content"><span>{statusReadFailed ? <Clock3 size={13}/> : "🕐"} {statusReadFailed ? "Could not load the latest community update" : previousUpdate ? "Waiting for a fresh community update" : "Waiting for the next community update"}</span></div></>}{!approved && <div style={{marginTop:12,fontSize:12,color:"var(--muted)"}}>{loggedIn ? "Player verification approval is required to submit updates." : <> <Link href="/signup" style={{color:"var(--red)",fontWeight:700}}>Request player verification</Link> to submit status updates.</>}</div>}</article>; })}</div>}<footer style={{fontSize:11,color:"var(--muted)",textAlign:"center",padding:"24px 0"}}>Community reported counts · Please verify in person</footer></main>;
}
